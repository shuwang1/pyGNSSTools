...
ephAssist, satXyzvt = GnssUtil.Get_GPS_Positioning_Assistance( updatedEphemeris, rcvXyz, epoch, __DEBUG = False ) 
...
print( "\tGnssUtil.Get_GPS_Positioning_Assistance():: From the location rcvXyz = ", rcvXyz, ", the %d visibles are " % ephVisibles )
...
localDir = os.getcwd() 
print( "\tThe current working directory is %s" % localDir )

dir_Code_Phase_Search = "~/Documents/Code_Phase_Search/"
status = subprocess.call( "cp  %s  %s" % ( satellitePositioningAssistanceFilename, dir_Code_Phase_Search ) , shell=True )
print( "\tcp  %s  %s; status=" % ( satellitePositioningAssistanceFilename, dir_Code_Phase_Search), status )

status = subprocess.call( "cp  %sCode_Phase_Search  ./" % ( dir_Code_Phase_Search ), shell=True )
print( "\tcp  %sCode_Phase_Search  ./; status=" % ( dir_Code_Phase_Search ), status )
...
print ( "\t[Initial1] With ", epoch_ , " %d SVs, the estimate is " % jj, eRcvXyzt, " and the error is %.2f m, norm(dRcvXyzt) = %.2f " %( obsError, sqrt( dRcvXyzt[0]*dRcvXyzt[0] + dRcvXyzt[1]*dRcvXyzt[1] + dRcvXyzt[2]*dRcvXyzt[2] ) ) ) 
...
rcvXyzt_, dRcvXyzt = GnssUtil.Get_GPS_Receiver_XyzPosition( updatedEphemeris,  [ epoch[0], epoch[1]+kk_ ], obsVisibleSats[0:jj ], obsCodeShifts[0:jj], rcvXyzt[0:3], __DEBUG = True )
obsErr      = sqrt( (rcvXyzt[0]-rcvXyzt_[0])**2 + (rcvXyzt[1]-rcvXyzt_[1])**2 + (rcvXyzt[2]-rcvXyzt_[2])**2 )
print( "\t[Initial2] jj = %d, res.x = %f, res.fun = %.2f, obsErr = %.2f" % ( jj, res.x, res.fun, obsErr ) )
...
for ii in range( - 100, 101, 1 ):
    ...
    for jj in range( max( 6, jj_-3 ), min( jj_+3, obsNumVisibles ), 1 ):
        ...
        if( obsError < 1000 ):
            print ( "\t\tWith ToA = %d s & %d visibles, the estimate is " % (epoch[1]+kk, jj), eRcvXyzt, " and the error is %.2f m, norm(dRcvXyzt) = %.2f" % (obsError, sqrt( dRcvXyzt[0]*dRcvXyzt[0] + dRcvXyzt[1]*dRcvXyzt[1] + dRcvXyzt[2]*dRcvXyzt[2] ) ) )
...
for ii in range( - 100, 101, 1 ):
    ...
    for jj in range( max( 6, jj_-2 ), min( jj_+2, obsNumVisibles ), 1 ):
        ...
        if( obsError < obsError_ ):
            obsError_, ii_, jj_ = obsError, kk, jj
        if( obsError < 100 ):
            print ( "\t\tWith %d s & %d visibles, the estimate is " % (epoch[1]+kk, jj), eRcvXyzt, " and the error is %.2f m, norm(dRcvXyzt) = %.2f" % ( obsError, sqrt( dRcvXyzt[0]*dRcvXyzt[0] + dRcvXyzt[1]*dRcvXyzt[1] + dRcvXyzt[2]*dRcvXyzt[2] ) ) )
